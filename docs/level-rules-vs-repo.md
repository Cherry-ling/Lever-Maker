...
- ✅ **16 冰冻/下方区域以现有关卡为准（2025）**：冰冻 = 仓库区域制（iceArea+CellType.Ice）为准；下方区域下层棋子 = layer=10 普通块（lv_65 实证）。
- ✅ **17 下方×无色叠放新规则（2025）**：**无色区域可叠在下方区域上层**——区内无棋子→解开下方区域→按无色区域规则走。⚠️ 452 关无重叠实例（同关 13 个均不相交），属编关扩展规则，生成时需可解性验证
